          ...
          {borrowings.length > 0 ? (
            borrowings.map((borrowing, index) => (
              <tr key={index}>
                ...
                <td>
                  <Button
                    onClick={() => handleConfirm(borrowing.borrowingID)}
                    disabled={borrowing.status !== "Đang chờ phê duyệt"}
                  >
                    Phê duyệt mượn
                  </Button>
                </td>
                <td>
                  <Button
                    onClick={() => handleConfirmReturn(borrowing.borrowingID)}
                    disabled={borrowing.status !== "Đang chờ phê duyệt trả"}
                  >
                    Phê duyệt trả
                  </Button>
                </td>
